...
echo "Начало генерации Kotlin API клиента..."
...
if [ -d "new/src/main/kotlin" ]; then
    cp -r new/src/main/kotlin/* src/main/kotlin
fi

# Копирование ApiClient.kt из .reserved с заменой
if [ -f ".reserved/ApiClient.kt" ]; then
    echo "Копирование ApiClient.kt из .reserved..."
    cp .reserved/ApiClient.kt src/main/kotlin/ru/testit/kotlin/client/infrastructure/ApiClient.kt
fi
...
echo "Генерация Kotlin API клиента завершена успешно!"
